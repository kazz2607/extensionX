...
import { broadcastQueueUpdate } from './utils.ts';

${getLines(60, 128)}
export { profileQueue, loadPersistedQueue, persistQueue, broadcastQueueUpdate, startNextInQueue };
`;
fs.writeFileSync('src/background/queue.ts', queueTs);

// 3. utils.ts (new)
const utilsTs = `// @ts-nocheck
import { tabState, downloadState } from './state.ts';
...
import { broadcastToPopup, broadcastToTab, sanitizeFolder } from './utils.ts';
import { showDownloadNotification } from './scraper.ts';
...
import { updateBadge, broadcastToPopup, updateFAB, broadcastFABState, sleep, waitForTabLoad } from './utils.ts';
...
import { profileQueue, persistQueue, startNextInQueue, broadcastQueueUpdate } from './queue.ts';
...
// 7. service-worker.ts (Entry Point)

...
export async function fetchHLS(m3u8Url, onProgress) {
  ...
  // 4. Fetch từng segment song song (max 4)
  const CONCURRENCY = 4;
  ...
  for (let i = 0; i < segments.length; i += CONCURRENCY) {
    ...
    const batchResults = await Promise.all(
      batch.map(async (segUrl, batchIdx) => {
        ...
        try {
          const res = await fetch(segUrl, { credentials: 'omit', cache: 'no-store' });
  ...
  return new Blob([combined], { type: 'video/mp2t' }); // MPEG-TS container
}

...
// @ts-ignore
      ...
      case 'FOLLOWING_SCROLL_PROGRESS': {
        ...
        if (statusEl) statusEl.textContent = payload.isHidden ? 'Tab hidden — pausing...' : `Scanning... (${payload.usersFound} found)`;
        // Animate progress bar (indeterminate: pulse between 20-90%)
        if (progressBar) {
          const pct = Math.min(20 + (payload.scrollCount % 10) * 7, 90);
      ...
      case 'FOLLOWING_SCROLL_DONE':
        cleanupSetScrolling(false);
        cleanupRenderResults(payload.users || [], payload.total || 0, payload.scrollCount || 0, payload.reachedEnd, payload.stopped);
        break;

      case 'FOLLOWING_SCROLL_ERROR':
        cleanupSetScrolling(false);
        cleanupSetStatus('error', `Error: ${payload.error}`);
...
// State
let _cleanupUsers: { username: string; displayName: string; order: number }[] = [];

function cleanupSetScrolling(isScrolling: boolean) {
...
function cleanupSetStatus(type: 'done' | 'error' | 'stopped', message: string) {
  const statusEl  = document.getElementById('cleanup-status');
  const statusDot = document.getElementById('cleanup-status-dot');
  const statusTxt = document.getElementById('cleanup-status-text');
  if (statusEl)  { statusEl.style.display = 'flex'; }
...
function cleanupRenderResults(
  ...
  cleanupSetStatus(stopped ? 'stopped' : 'done', statusMsg);
  ...
  // Render top users (max 50 shown, rest in CSV)
...
function initCleanupPanel() {
  ...
  const btnStart  = document.getElementById('btn-cleanup-start')  as HTMLButtonElement | null;
  const btnStop   = document.getElementById('btn-cleanup-stop')   as HTMLButtonElement | null;
  const btnCopy   = document.getElementById('btn-cleanup-copy')   as HTMLButtonElement | null;
  const btnCsv    = document.getElementById('btn-cleanup-csv')    as HTMLButtonElement | null;

  if (!urlInput || !btnStart) return;

  // Auto-detect: nếu tab hiện tại đang ở /following, điền sẵn URL
  ...
  btnStart.addEventListener('click', async () => {
    ...
    const res: any = await sendBG('START_FOLLOWING_SCROLL', { targetUrl: rawUrl });
    if (res?.error) {
      showToast(`Lỗi: ${res.error}`, 'error');
      return;
    }
    // UI update is handled via FOLLOWING_SCROLL_STARTED broadcast
  ...
  if (btnCopy) {
    btnCopy.addEventListener('click', () => {
      if (_cleanupUsers.length === 0) { showToast('Chưa có dữ liệu', 'error'); return; }
      const text = _cleanupUsers.map(u => `@${u.username}`).join('\n');
      navigator.clipboard.writeText(text).then(() => {
        showToast(`✓ Copied ${_cleanupUsers.length} usernames`, 'success');
      }).catch(() => {
  ...
  if (btnCsv) {
    btnCsv.addEventListener('click', () => {
      ...
      const usernameMatch = urlRaw.match(/\/([A-Za-z0-9_]+)\/following/);
      const targetUser = usernameMatch?.[1] || 'unknown';

      const header = 'rank,username,displayName,profileUrl\n';
      const rows = _cleanupUsers.map((u, i) =>
      ...
      showToast(`✓ Exported ${_cleanupUsers.length} users to CSV`, 'success');
    });
  }
}
